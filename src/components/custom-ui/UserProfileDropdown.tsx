...
import { Avatar, AvatarFallback, AvatarImage } from '../ui/avatar'; // Sesuaikan path sesuai lokasi Anda
...
export default function UserProfileDropdown() {
    ...
    useEffect(() => {
        function handleClickOutside(event: MouseEvent) { // Explicitly type event as MouseEvent
    ...
    return (
        ...
        <div className="relative" ref={dropdownRef}> 
            ...
            >
                ...
                {/* Dropdown Icon */}
                <svg className={`h-4 w-4 text-white ml-1 hidden sm:block transition-transform duration-200 ${isOpen ? 'rotate-180' : 'rotate-0'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
            ...
            {isOpen && (
                <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-gray-800 rounded-md shadow-xl border border-gray-200 dark:border-gray-700 z-30 transform origin-top-right animate-fade-in">
                    ...
                    <div className="py-1">
                        ...
                        {/* Opsi Role-Based */}
                        {userRole === 'ADMIN' && (
                            <Link to="/dashboard/admin" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-700" onClick={() => setIsOpen(false)}>
                                Dashboard Admin
                            </Link>
                        )}
                        
                        <Link to="/profile" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-700" onClick={() => setIsOpen(false)}>
                            Profil Saya
                        </Link>
                        
                        <Link to="/settings" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-700" onClick={() => setIsOpen(false)}>
                            Pengaturan
                        </Link>
